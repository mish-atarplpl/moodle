...
class csv_import_reader {
	...
	// MDL-41116 fgetcsv works only with ASCII characters so use fgets
	while ($fdata = fgets($fp)) {
            $fgetdata = explode($csv_delimiter, $fdata);
            foreach ($fgetdata as $key => $value)   {
                $fgetdata[$key] = trim($value);
            }
